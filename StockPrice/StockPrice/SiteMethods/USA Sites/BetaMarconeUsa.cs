...
{
    public sealed class BetaMarconeUsa
    {
        private const string Source = "marcone.com";
        private const string ClassSource = "BetaMarconeUsa";
        ...
        public static async void Parsing(DatabaseTotalResults request, List<MainPriceResponse> mainPriceResponsesList)
        {
            ...
            var authMarcone = MarconeAuthUsa.MarconeAuthCookie;
            ...
            {
                redirect = CustomHttpClass.CheckRedirectGet(@$"https://beta.marcone.com/Home/SearchPartModelList?searchString={search}&Type=Part", coockies: authMarcone, acceptencoding: "none", selected_proxy: MarconeAuthUsa.SelectedProxy);
            ...
            else //If you didn't hit the redirect - you need to analyze the output
            {
                ...
                try
                {
                    search_result = CustomHttpClass.GetToString(@$"https://beta.marcone.com/Home/SearchPartModelList?searchString={search}&Type=Part", coockies: authMarcone, acceptencoding: "none", selected_proxy: MarconeAuthUsa.SelectedProxy);
            ...
            foreach (var totalUrlTaken in totalUrlsList)
            {
                ...
                try //We're getting a page response
                {
                    while (true)
                    {
                        response = CustomHttpClass.GetToString(totalUrl, coockies: authMarcone, acceptencoding: "none", selected_proxy: MarconeAuthUsa.SelectedProxy);
                ...
                try //Request for information on product availability
                {
                    ...
                    availabilityResponse = CustomHttpClass.PostToString(@"https://beta.marcone.com/Landing/GetSSBStockInfo",
                        ...
                        data: rp,
                        acceptencoding: "none", selected_proxy: MarconeAuthUsa.SelectedProxy);
                }
                catch (Exception ex)
                {
                    await ResponseCreator.MakeErrorLog(con: con,
                ...
                if (availabilityResponse != null)
                {
                    ...
                    if (availabilityRegex.Any())
                    {
                        ...
                        if (availabilityAllResponse != null) //A list of all locations.
                        {
                            ...
                            {
                                Console.WriteLine("BetaMarcone error locations");
