...
{
    public sealed class ReliablePartsAuthCanada
    {
        ...
        {
            Console.WriteLine("First auth ReliableParts [Canada]..");
            ...
            con.Open();
            var reliablepartsAuthInfo = con.QueryFirstOrDefault<DatabaseAuthData>($"SELECT * FROM auth_data WHERE `resource`='reliableparts';");
            con.Close();
            ...
            try
            {
                response = CustomHttpClass.PostToString(url: @"https://prodapi.reliableparts.net/ca/accountapp/v1/security/api/auth/login",
                    contentType: "application/json",
                    jsonData: authDataString);
            }
            catch
            {
                Console.WriteLine("Error during auth ReliableParts [Canada] Stage - 0");
            }
            ...
            if (response != null)
            {
                ...
                if (respInfo.AccessToken != null)
                {
                    ...
                    AuthReady = true;
                    Console.WriteLine("Auth ReliableParts [Canada] Completed!");
                }
            }
        ...
        private static void ReAuthReliableParts()
        {
            while (true)
            {
                ...
                Console.WriteLine("Re-auth ReliableParts [Canada]..");
                var cs = @$"Server={AppSettings.Current.Database.Host};Port={AppSettings.Current.Database.Port};User={AppSettings.Current.Database.Login};Database={AppSettings.Current.Database.Database};Password={AppSettings.Current.Database.Password}";
                using var con = new MySqlConnection(cs);
                con.Open();
                var reliablepartsAuthInfo = con.QueryFirstOrDefault<DatabaseAuthData>($"SELECT * FROM auth_data WHERE `resource`='reliableparts';");
                ...
                if (response != null)
                {
                    ...
                    if (resp_info.AccessToken != null)
                    {
                        ...
                        AuthReady = true;
                        Console.WriteLine("Re-auth ReliableParts [Canada] Completed!");
                    }
                }
            }
        }
