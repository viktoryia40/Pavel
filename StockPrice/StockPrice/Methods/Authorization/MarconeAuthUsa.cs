...
namespace StockPrice.Methods.Authorization
{
    ...
    public class MarconeAuthUsa
    {
        ...
        {
            Console.WriteLine("First auth Marcone [USA]..");
            ...
            con.Open();
            var marconeAuthInfo = con.QueryFirstOrDefault<DatabaseAuthData>($"SELECT * FROM auth_data WHERE `resource`='marconeUSA';");
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"Error auth Marcone [USA], stage - 2. Exception: {ex.Message}");
            }
            ...
            if (_jres.Result.ToString().Equals("True"))
            {
                ...
                {
                    Console.WriteLine($"Error auth Marcone [USA], stage - 3. Exception: {ex.Message}");
                }
            }
            else
            {
                Console.WriteLine("Incorrect password or login Marcone [USA].");
            }
        ...
        public static void ReAuthMarcone()
        {
            while (true)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Console.WriteLine($"Error re-auth Marcone [USA], stage - 1. Exception: {ex.Message}");
                }
                ...
                catch (Exception ex)
                {
                    ...
                    Console.WriteLine($"Error re-auth Marcone [USA], stage - 2. Exception: {ex.Message}");
                }
                ...
                if (jres.Result.ToString().Equals("True"))
                {
                    ...
                    {
                        Console.WriteLine($"Error re-auth Marcone [USA], stage - 3. Exception: {ex.Message}");
                    }
                }
                else
                {
